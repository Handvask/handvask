...
from .Models import DBHandler, Solver, Sys_admin, User, Mzn_instance, Dzn_instance, Run, Run_status
...
class Test_get_runs:
    ...
    def make_run_data_for_test():
        ...
        Run(user=1, solvers=solver, mzn_instance=mzn, dzn_instance=dzn, status=Run_status.EXCEPTION)
        
        # Run instance for user2
        mzn2=Mzn_instance(user=3, contents="test")
        dzn2=Dzn_instance(user=3, contents="test")
        Run(user=3, solvers=solver, mzn_instance=mzn2, dzn_instance=dzn2, status=Run_status.EXCEPTION)
        commit()
